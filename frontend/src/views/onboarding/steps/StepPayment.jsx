  ...
  const handlePayClick = async () => {
    ...
    try {
      ...
      if (isLoggedIn) {
        ...
        try {
          const { user, jwt: newToken } = await post('/api/add/user/onboarding', {
          ...
          // Compte déjà existant — login avec les credentials fournis
          const loginRes = await fetch(`${base}/api/login`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ email: userData.email, password: userData.password }),
          });
          if (!loginRes.ok) throw Object.assign(new Error('409'), { status: 409 });
          const loginData = await loginRes.json();
          sessionToken = loginData.jwt;
          sessionUser  = loginData.user;
          compte_id    = loginData.user.id;
        }
      ...
      <div className="flex-1 w-full max-w-xl mx-auto px-5 py-7 lg:px-8 lg:py-10 flex flex-col">
        ...
        <div className="mt-auto pt-2">
          <div className="flex items-center justify-between mb-4 pt-4 border-t border-border">
            ...
            <span className="text-xl font-black text-anthracite">{priceLabel}</span>
          </div>
